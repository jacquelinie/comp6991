    ...
    loop {
        ...
        match manager.accept_new_connection() {
            Connection::NewConnection { mut reader, mut writer } => {
                ...
                // Extract the sender's name using the custom Reader trait
                let sender_name = get_sender_name(&mut reader);

                // Obtain or insert the SenderHandle for this sender
                ...
                println!("Connection established with {}", sender_name);

                // Try reading an initial message before spawning the thread
                match reader.read_message() {
                    ReadMessageResult::Message(msg) => {
                        // Handle the initial message
                        println!("Processing initial command...");
                        handle_initial_message(msg, &sender_handle, &mut reader, &mut writer);
                    }
                    _ => {
                        // Spawn a thread to handle further messages from this connection
                        thread::spawn(move || {
                            println!("Handling connection in new thread...");
...
fn handle_connection(
    sender_handle: Arc<SenderHandle>,
    recv: &mut dyn Reader,
    send: &mut dyn Writer
    ...
    loop {
        match recv.read_message() {
            ReadMessageResult::Message(msg) => {
                println!("Processing command in connection...");
                let _lock = sender_handle.order_mutex.lock().unwrap();

                // Handle the message
                let reply = process_command(msg);
...
fn process_command(msg: String) -> Reply {
        ...
        Ok(command) => match command {
            ...
            Command::Set { cell_identifier, cell_expr } => {
                if let Some(reply) = handle_set(&cell_identifier, &cell_expr) {
                    ...
                    Reply::Error("Set command failed".to_string())
                }
            }
        },
...
// Helper function to process the initial message
fn handle_initial_message(
    ...
    match send.write_message(reply) {
        ...
        WriteMessageResult::Err(e) => eprintln!("Error sending reply: {}", e),
    }
}
